...
- **Arquitectura API separada:** el API vive en `api.promptometer.tech` (subdominio dedicado, sin proxy de Cloudflare para que el rate-limiting reciba IPs reales). Las llamadas desde el cliente JS se resuelven vía `js/api-config.js` (detecta el ambiente). Host-check en `api/index.js` bloquea cualquier host no autorizado con 403.
- **Setup Cloudflare/Vercel pendiente (manual):** crear registro `api` CNAME → `cname.vercel-dns.com` en nube gris; añadir `api.promptometer.tech` en Vercel → Settings → Domains.
