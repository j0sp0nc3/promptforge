...
- [x] **Reglas del Motor Actualizadas a la Realidad 2026 (auditoría de obsolescencia, 2026-08-29)**: (1) `AP046`/`assumesCapability` modernizado — navegar, ejecutar código y datos en vivo YA son capacidades estándar en 2026: la penalización ahora aplica solo si el prompt NO declara el contrato de herramientas (`hasToolContracts`: bloque `<tools>`, function calling, MCP) que las habilita; textos i18n ES/EN actualizados. (2) Regex de años `(2024|2025|2026)` → `20[2-9]d` (dinámica a futuro) y ejemplo de sugerencia sin año hardcodeado. (3) Knowledge Hub: +4 técnicas 2025-26 (**Context Engineering**, **MCP — Model Context Protocol**, **Test-Time Compute**, **Patrones Agénticos de Andrew Ng**) → 17 técnicas; URL beta obsoleta de la referencia del repo corregida a promptometer.tech. Auditoría adicional: adversarial.js y domain-analyzer.js sin supuestos datados (postCutoffYear ya es dinámico y matizado por RAG).
...
> - **Establecimiento de Regla Inviolable de Puerta de Validación Humana (Human-in-the-Loop):** Se actualizó `AGENTS.md` y `.agents/AGENTS.md` prohibiendo estrictamente que el agente realice merge o push a `main` por su cuenta. Todo cambio debe compilarse, probarse en local (`34/34 PASS`), pushearse exclusivamente a `dev` (`https://promptometer.vercel.app/`), y **DETENERSE**. La fusión a `main` solo ocurre cuando el USUARIO apruebe explícitamente los cambios en `dev` y ordene la promoción a producción.
