...
>   - **Estado Inicial Protoplanetario**: En reposo muestra una **protoestrella** (núcleo azul/blanco brillante) rodeada por un disco giratorio de **polvo estelar protoplanetario** (`protoplanetaryDisk`) con la indicación *"Esperando evaluación…"*.
>   - **Rediseño Físico de Transición Astronómica (Acccreción -> Trazado)**: Se mejoró la cinemática de la animación tridimensional: en la **Fase 1 (Condensación Gravitacional)** las partículas de polvo protoplanetario colapsan e implotan hacia los 8 focos orbitales para encender y hacer crecer los 8 planetas 3D mientras las líneas de órbita permanecen invisibles (`opacity = 0`); posteriormente en la **Fase 2 (Ignición y Trazado)**, los planetas ya formados comienzan su traslación orbital y **van trazando/generando las 8 órbitas luminosas a su paso** a medida que el polvo restante se disipa por completo.
>   - **Paridad Estricta de Colores 8D**: Los 8 planetas 3D ahora mantienen su **color característico distintivo** idéntico al de las tarjetas de métricas inferiores (`Clarity` `#00e5ff`, `Specificity` `#7c3aed`, `Structure` `#f59e0b`, `Robustness` `#10b981`, `Context` `#ec4899`, `Output` `#38bdf8`, `CoT` `#f97316`, `Safety` `#ef4444`).
>   - **Navegación Interactiva por Clic (Planeta 3D y Tarjeta de Métrica)**: Al hacer clic sobre cualquier planeta 3D o tarjeta de métrica numerada, la página se desplaza suavemente hacia la sección de detalles (pestaña *"Dimensiones"*), expande la tarjeta objetivo y la resalta con una animación de pulso luminoso neón (`highlight-pulse`).
>   - **Corrección de Solapamiento 3D del Sol en Modo Luna 🌙**: Se eliminó el fondo opaco y borde HTML sólido de `.central-score-box` en la variante `theme-editorial` de CSS. Ahora, tanto en **Modo Luna 🌙** como en **Modo Black Hole 🕳️**, la caja de score central es 100% transparente y no bloquea el motor WebGL 3D, permitiendo que el Sol permanezca orgánicamente en el centro y los planetas 3D en traslación orbiten de forma natural por delante y por detrás del cuerpo estelar.
>   - **Etiquetas Flotantes Permanentes por Planeta 3D**: Cada planeta 3D ahora proyecta en tiempo real su etiqueta con nombre de la dimensión y score (`Seguridad 5.0`, `Claridad 8.5`, etc.) flotando permanentemente sobre él mientras orbita en el espacio 3D. Las etiquetas se adaptan dinámicamente tanto al **Modo Black Hole 🕳️** (insignias neón) como al **Modo Luna 🌙** (papel crema e impresiones finas), manteniendo interactividad total al hacer clic.
>   - **Pausa Interactiva al Posicionar el Cursor (Hover)**: Al posicionar el cursor sobre cualquiera de los planetas 3D en órbita, **la rotación orbital y la rotación propia se pausan inmediatamente**, permitiendo inspeccionar o hacer clic con total facilidad. Además, el planeta pausado se escala sutilmente a **1.25x** como respuesta táctil inmediata.
>   - **Reinicio Automático**: Al presionar "Limpiar" en el editor, el sistema solar se reinicia suavemente a su estado inicial protoplanetario.
>   - **Visualización Limpia del Score**: El puntaje (`88.5`) y su letra (`A+ EXCELLENT`) flotan de forma transparente y orgánicamente en el centro del Sol 3D, **sin marcos ni tarjetas rectangulares superpuestas**.
> - **Integración del Objetivo del Prompt (`options.objective`)**: Se conectó el selector de Objetivo (`#prompt-objective-select`) con `Analyzer.analyze(prompt, { objective })`, `Signals.weightsFor(type, objective)` y `Rewriter.improve(prompt, analysis, { objective })`. Ahora, al seleccionar un objetivo específico (ej. *Generación de Código*, *Análisis & Razonamiento*, *Esquema JSON*, *Seguridad & RAG* o *Redacción Creativa*), la evaluación aplica automáticamente la tabla de pesos y reglas especializadas para ese propósito.
> - **Corrección de Traducción en Hallazgos (`js/i18n.js`)**: Se integraron las llaves faltantes `tooShort` y `expandSugg` dentro de `analyzer.specificity` en los diccionarios de español e inglés, corrigiendo la fuga de texto crudo (`analyzer.specificity.tooShort`).
> - **Pruebas de Estrés**: 22/22 PASS en `test_edge_cases.js`. Paridad i18n y profundidad de renderizado 3D en ambos temas verificados al 100% en local.
